...
public interface PersonApi {
	...
	@ApiOperation(value = "get person reputation", notes = "", response = BigDecimal.class, tags = { "Person", })
	@ApiResponses(value = { @ApiResponse(code = 200, message = "", response = BigDecimal.class) })
	@RequestMapping(value = "/person/{uuid}/reputation/{communityid}/{valuetypeid}/{reputationtype}", produces = {
	...
	ResponseEntity<BigDecimal> gETPersonUuidReputationCommunityidValuetypeidReputationtype(
			...
			@ApiParam(value = "") @RequestBody ReputationCalculation body

	);

	@ApiOperation(value = "get Person", notes = "", response = Person.class, tags = { "Person", })
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Status 200", response = Person.class) })
	@RequestMapping(value = "/person/{uuid}", produces = { "application/json" }, method = RequestMethod.GET)
	...
	@RequestMapping(value = "/person", consumes = { "application/json" }, method = RequestMethod.PUT)
